...
#
# Common Voice: cv17_da needs a local manifest of the leaderboard's 2,756 clips,
# built from Common Voice 27.0. Fetch it once and export CV_DATA_DIR, else cv17 is skipped:
...
#   ORDBOGEN_RPM=230 RUN_API=1 bash scripts/run_benchmark.sh  # Tier 1 pacing (Tier 0 = 120/min)
...
LOCAL_MODELS=(
  ...
  "MediaCatch/xls-r-300m-danish-mc-v2|wav2vec2|"
...
API_MODELS=(
  ...
  "gpt-transcribe|azure-openai|--access proprietary --azure-openai-api-key ${AZURE_OPENAI_API_KEY:-} --azure-openai-endpoint ${AZURE_OPENAI_ENDPOINT:-}"
